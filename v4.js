...
export const V4 = TEST_BINDGROUP + /* wgsl */`

    var<workgroup> workgroupTestSyncValue: u32;
    ...
    fn main( @builtin(global_invocation_id) GlobalThreadID : vec3<u32>, @builtin(workgroup_id) WorkGroupID : vec3<u32>, @builtin(local_invocation_index) LocalInvocationIndex : u32,  @builtin(local_invocation_id) LocalInvocationId : vec3<u32> )
    {
        ...
        // Better barrier placement than v1, also makes sure writes reach L2 before continuing
        storageBarrier();

        // Better sharing of the result with all threads in the threadgroup than v2
        if (LocalInvocationIndex == 0)
        {
            workgroupTestSyncValue = atomicAdd(&testSyncAndErrorCount[0], 1);
        }

        workgroupBarrier();

        // atomic sync
        if(workgroupTestSyncValue != (4096 - 1))
        {
            return;
        }
